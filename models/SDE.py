...
class SDE(abc.ABC):
    ...
    def reverse_sde_step_mean(self, x, score, t):
        ...
        # else:
        #     return self.gc(x - self.sde_reverse_drift(x, score, t), t)
...
class IRSDE(SDE):
    ...
    def __init__(self, opt, device=None):
        super().__init__(opt['T'], device)
        # self.train_optimal_t = opt['train_optimal_t']
        # self.test_optimal_t = opt['test_optimal_t']
        # self.type = opt['type']
        # assert self.type in ['one', 'range', 'whole', None]
        self.max_sigma = opt['max_sigma'] / 255 if opt['max_sigma'] >= 1 else opt['max_sigma']
        self._initialize(self.max_sigma, opt['T'], opt['schedule'], opt['eps'])
    ...
    def set_model(self, model, gc=None):
        self.model = model
        self.gc = gc
    ...
    # predicted x0 is obtained from Xt and t
    def get_x0(self, states, noise, timesteps):
        states = states.to(self.device)
        noise = noise.to(self.device)
        noise_level = self.sigma_bar(timesteps)
        x0 = (states - noise * noise_level - self.mu) / torch.exp(-self.thetas_cumsum[timesteps] * self.dt) + self.mu
        return x0.to(torch.float32)

    def score_fn(self, x, t, **kwargs):
        # need to pre-set mu and score_model
        # _, noise = self.model(x, self.mu, self.dark, t, **kwargs) 
        noise = self.model(x, self.mu, t, **kwargs)

        return noise, self.get_score_from_noise(noise, t)

    def noise_fn(self, x, t, **kwargs):
        # need to pre-set mu and score_model
        # return self.model(x, self.mu, self.dark, t, **kwargs)
    ...
    def reverse_sde_imp_val(self, xt, T=-1, save_states=False, save_dir='sde_state', r=100, center=1, **kwargs):
        ...
        for t in tqdm(reversed(range(center, T + 1))):

            noise, score = self.score_fn(x, t, **kwargs)
            
            x0_hat = self.get_x0(x, noise, t)
            ...
            if save_states: # only consider to save 100 images
                ...
                if t % interval == 0:
                    idx = t // interval
                    os.makedirs(save_dir, exist_ok=True)
                tvutils.save_image(x.data, '{}/state_{:0>2d}.png'.format('results/state', t), normalize=False)
                # tvutils.save_image(noise.data, '{}/noise_{:0>2d}.png'.format('results/region', t), normalize=False)
        return x0_hat

    def reverse_sde_original_sample(self, xt, T=-1, save_states=False, save_dir='sde_state', r=70, **kwargs):
        # print(T)
        T = self.T if T < 0 else T
        x = xt.clone()
        ...
        for t in tqdm(reversed(range(1, T + 1))):
            ...
            if save_states: # only consider to save 100 images
                ...
                if t % interval == 0:
                    # idx = t // interval
                    os.makedirs(save_dir, exist_ok=True)
                tvutils.save_image(x.data, '{}/state_{:0>2d}.png'.format(save_dir, t), normalize=False)
        return x
    

    def reverse_sde_improved_sample(self, xt, T=-1, save_states=False, save_dir='sde_state', r=70, **kwargs):
        # print(T)
        T = self.T if T < 0 else T
        x = xt.clone()
    ...
    def generate_random_states(self, x0, mu):
        ...
        #     timesteps = torch.randint(self.train_optimal_t, self.T + 1, (batch, 1, 1, 1)).long()
        # else:
        #     timesteps = torch.randint(1, self.T + 1, (batch, 1, 1, 1)).long()
